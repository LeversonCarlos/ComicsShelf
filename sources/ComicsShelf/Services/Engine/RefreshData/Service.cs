...
namespace ComicsShelf.Engine.RefreshData
{
   public class Service
   {
      ...
      {
         using (var log = new Helpers.InsightsLogger($"{library.Type} Items Refreshing"))
         {
            try
            {
               ...
               var searchItems = await Drive.BaseDrive
                  ...
                  .SearchItems(library);
               Helpers.Insights.TrackEvent($"{library.Type} Engine");
               ...
               log.Add("Itens", $"{(searchItems?.Length ?? 0)}");
            ...
            catch (Exception ex) { log.Add(ex); Helpers.Insights.TrackException(ex); Helpers.Notify.Message(library, $"Error while refreshing data: {ex.Message}"); }
         }
      }
