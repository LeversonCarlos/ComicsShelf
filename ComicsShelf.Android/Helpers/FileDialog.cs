   ...
   public class FileDialog
   {
      ...
      public FileDialog(Context context, FileSelectionMode mode)
      {
         ...
         try
         {
            ...
            _mSdcardDirectory = new Java.IO.File(_mSdcardDirectory).CanonicalPath;
         }
      ...
      public async Task<string> GetFileOrDirectoryAsync(String dir)
      {
         ...
         try
         {
            dir = new Java.IO.File(dir).CanonicalPath;
         }
         catch (IOException ioe)
         {
            return _result;
         ...
         AlertDialog.Builder dialogBuilder = CreateDirectoryChooserDialog(dir, _mSubdirs, (sender, args) =>
          {
             ...
             if ((new Java.IO.File(_mDir).IsFile)) // If the selection is a regular file
             {
                _mDir = mDirOld;
      ...
      {
         var newDirFile = new Java.IO.File(newDir);
